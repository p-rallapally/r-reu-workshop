...
R is a very powerful tool for scientific analysis and data visualization. Let's suppose that you are a penguin researcher, and you have collected data from different species of penguins by measuring their anatomical features such as flipper length, body mass, beak size etc. Now you want to investigate if bill length and bill depth are related to each other.

![](images/culmen_depth.png){width="405"}

Copy the code below and execute it in the console to find out!

```{r, plot1, exercise=TRUE, exercise.eval = FALSE, message=FALSE, warning=FALSE, results='hide'}
...
`View()` is a useful function to view your data. Try it as below!

```{r, read2, exercise=TRUE, exercise.eval = FALSE}
View(my_data)
```

### EXERCISE

Read in (import) the `ev_police_jan.csv` data file that's in the data directory, and view it.
...
-   Save your file.
    ...
    ```{r, code2, exercise=TRUE, exercise.eval = FALSE}
    ```
...
Now that you are familiar with most of the basics, let's come back to the Penguin dataset we saw at the beginning. The data is from <https://github.com/allisonhorst/palmerpenguins>. It's been included in an R package.
...
There's a data frame in the package called `penguins`. Like with functions in a package, we can use it once we've loaded the package with the library command above. View this dataframe.
...
Don't forget to practice the homework exercises in the `Practice Day 1.R` file!
