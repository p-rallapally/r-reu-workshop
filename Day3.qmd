...
```{r}
police %>% 
  mutate(vehicle_age = 2024 - vehicle_year) %>%
...
police %>% 
  mutate(
    ...
    vehicle_age_norm = ifelse(
      vehicle_age < threshold,  # ifelse test condition
      0,  # value if true
      vehicle_age
      )  # value if false
         ) %>%  
...
```{r}
police <- mutate(police, vehicle_age = 2024-vehicle_year)
```
